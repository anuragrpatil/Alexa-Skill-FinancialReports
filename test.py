...
def get_welcome_response():
    ...
    speech_output = "Welcome, I can help you out with information about conducted trades"
...
def get_help():
    session_attributes = {}
    card_title = "Help"
    url = 'https://raw.githubusercontent.com/anuragrpatil/Alexa-Skill-FinancialReports/master/2019_full_prem-converted.csv'
...
def getInfo(intent, session):
    ...
    namelookupmore = intent['slots']['MoreInfo']['value'].lower()
    name = str(namelookup)
    more = str(namelookupmore)
    today = (time.strftime("%m/%d/%Y"))
    url = 'https://raw.githubusercontent.com/anuragrpatil/Alexa-Skill-FinancialReports/master/InputAnnualReport.csv'
    ...
    for row in contents:
        for element in row:
            if name in element.lower():
                if more in element.lower():
                    title = row[0]
                    description0 = row[1]
                    description1 = summarize(row[2], word_count = 100)
                    # description2 = row[2]
                    # description3 = row[8]
                    # description4 = row[13]
                    # description5 = row[4]
                ...
                # summary = str(description1) + " received sales credit of " + str(description2) + " on " + str(date) + " The client was " + str(description3) + " with a  " + str(description5) + " Trade through " +str(description4) + " by " + str(description0)
                summary = str(description) + str(description0) + str(description1)
                records.append(summary)
    temp1 = '. '.join(records)

    speech_output = str(temp1) + " Any additional information that you require?"
